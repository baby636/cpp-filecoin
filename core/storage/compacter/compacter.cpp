...
namespace fc::storage::compacter {
  ...
  void CompacterIpld::_start() {
    ...
    auto genesis{ts_load->loadw(ts_main->chain.begin()->second).value()};
    start_head = ts_load->loadw(ts_main->chain.rbegin()->second).value();
    ts_lock.unlock();
  ...
  void CompacterIpld::_flow() {
    while (true) {
      std::shared_lock ts_lock{*ts_mutex};
      auto head1{ts_load->loadw(ts_main->chain.rbegin()->second).value()};
      ...
      _queueLoop();
      ts_lock.lock();
      auto head2{ts_load->loadw(ts_main->chain.rbegin()->second).value()};
      ts_lock.unlock();
  ...
  void CompacterIpld::_headersBatch() {
    ...
    --it;
    headers_top = ts_load->loadw(it->second).value();
    headers_top_key.setCbor(headers_top->key.cids());
  }
  ...
  void CompacterIpld::_finish() {
    std::unique_lock vm_lock{*interpreter->mutex};
    std::unique_lock ts_lock{*ts_mutex};
    auto head{ts_load->loadw(ts_main->chain.rbegin()->second).value()};
    auto ts{head};
    ...
    for (auto &branch : *ts_branches) {
      ...
      for (auto it : branch->chain) {
        auto ts{ts_load->loadw(it.second).value()};
        for (auto &block : ts->blks) {
